...
func NewReporter(user *types.User) *Reporter {
	return &Reporter{user: user}
}
...
func (r *Reporter) GenerateEmbeddings(prompt string) (embeddings pgvector.Vector, err error) {
	...
	var client *vai.PredictionClient
	if client, err = vai.NewPredictionClient(ctx, option.WithEndpoint(_vaiEndpoint)); err != nil {
		return
	}
	defer client.Close()
	...
	if resp, err = client.Predict(ctx, req); err != nil {
		return
	}
...
func (r *Reporter) GenerateDailyReport(data *UserData) (report *types.Report, err error) {
	...
	if response, err = chatSession.SendMessage(ctx, genai.Text(stringData)); err != nil {
		return nil, err
	}
	report = &types.Report{
		StartDate:  data.Date,
